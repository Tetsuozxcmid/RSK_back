        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Failed to fetch users: Status {response.status_code}, Body: {response.text}")
                    return []
            except Exception as e:
    ...
    async def update_user_learning_status(self, user_id: int, learning: bool) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    print(f"Successfully updated learning status for user {user_id} to {learning}")
                    
                    return True
                else:
                    print(f"Failed to update learning status for user {user_id}: Status {response.status_code}, Body: {response.text}")
                    return False
            except Exception as e:
                print(f"Exception while updating learning status for user {user_id}: {e}")
                return False
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(user_data, list) and len(user_data) > 0:
                        ...
                        print(f"Unexpected response format for user {user_id}: {user_data}")
                        return None
                else:
