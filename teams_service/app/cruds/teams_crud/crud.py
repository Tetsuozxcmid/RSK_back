...
class TeamCRUD:
    ...
    async def can_user_join_team(db: AsyncSession, team_id: int, user_id: int):
        print(f" DEBUG can_user_join_team: Called with user_id={user_id}, team_id={team_id}")
        ...
        if user_role == "student":
            ...
            if composition["students"] == 0 and composition["total"] + 1 == 4:
                return False, "Cannot add teacher - team must have exactly 1 student. No room left for student."
    ...
    async def create_team(db: AsyncSession, team_data, leader_id: int):
        ...
        try:
            ...
            org_info = await OrgsClient.get_organization_info(team_data.organization_name)
            if not org_exists:
    ...
    async def join_team(db: AsyncSession, team_id: int, user_id: int):
        ...
        print(f"DEBUG join_team: After can_user_join_team, can_join={can_join}, message={message}")
    ...
    async def analyze_team_composition(db: AsyncSession, team_id: int):
        ...
        print(f"DEBUG analyze_team_composition: Team {team_id} members: {[m.user_id for m in members]}")
        
        student_count = 0
        teacher_count = 0
        
        for member in members:
            print(f"DEBUG analyze_team_composition: Getting role for member {member.user_id}")
            role = await TeamCRUD.get_user_role(member.user_id)
            if role == "student":
                student_count += 1
            elif role == "teacher":
