            ...
            async for message in queue_iter:
                async with message.process():
                    try:
                        ...
                        logger.info(
                            f"[AUTH CONSUMER] Updating role for user {user_id}: {old_role} -> {new_role}"
                        )
                        ...
                        async with async_session_maker() as session:
                            ...
                            if user:
                                ...
                                logger.info(
                                    f"[AUTH CONSUMER] Role updated for user {user_id} to {new_role}"
                                )
                            else:
                                logger.error(
                                    f"[AUTH CONSUMER] User {user_id} not found"
                                )
